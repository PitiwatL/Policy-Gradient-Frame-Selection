...
from Utils.ImageDataset import ImageDataset1, ImageDataset2, ImageDataset3
from Utils.skim_net import SkimNetwork, EvalNetwork, PolicyNetwork
...
train_dataset = ImageDataset3(data_paths = X_train,
                             labels     = y_train,
...
val_dataset   = ImageDataset3(data_paths = X_val, 
                             labels     = y_val,
                             transform  = (train_transform1, train_transform2),
                             size = frame_size, 
...
skimNetwork = SkimNetwork(128)
evalNetwork = EvalNetwork(128)

skimNetwork = skimNetwork.to(device)
skimNetwork = torch.nn.DataParallel(skimNetwork)

evalNetwork = evalNetwork.to(device)
evalNetwork = torch.nn.DataParallel(evalNetwork)

skimNetwork.load_state_dict(torch.load('/project/lt200210-action/OCS_Sampler/weights/Skim_ResNet18_LSTM_64_30_60Classes.pt'))
                       
print("Load Skim weights complete!")

# evalNetwork.load_state_dict(torch.load('/project/lt200210-action/OCS_Sampler/weights/Eval_ResNet18_LSTM_224_20.pt'))

# print("Load Eval weights complete!")

skimNetwork.module.cf3 = nn.Identity() # pop the last layer out!

model_base = PolicyNetwork(skimNetwork, evalNetwork)
...
for eph in range(epochs):
    ...
    for b, (X_train1, X_train2, y_train) in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        output = model(X_train1, X_train2, 'Test')
        y_pred = output
    ...
    with torch.no_grad():
        for b, (X_val1, X_val2, y_val) in enumerate(val_loader):
            ...
            out_val = model(X_val1, X_val2, 'Test')
    ...
    if np.mean(loss_epoch_val) <= min(val_loss_his):
        ...
        if eph > 0 :
            ...
            torch.save(model.state_dict(), 
                       '/project/lt200210-action/OCS_Sampler/weights/Policy_224_5_B64.pt')
...
print('Save Complete on Policy on the flies cross_view 64!')